...
export const Tramos = () => {
 // Base de datos
  ...
  const getdata = async (FechaIncial, FechaFinal) => {
    ...
    const data = await axios.post("http://localhost:8080/tramos", body);
    ...
    <>
      ...
      <ResponsiveContainer width="50%" aspect={2}>
        ...
        >
          ...
          <Tooltip />
          <Legend />
          <Bar dataKey="consumo" fill="#9500F9" />
          <Bar dataKey="costo" fill="#32AB32" />
          <Bar dataKey="perdidas" fill="#ee7820" />
        </BarChart>
      </ResponsiveContainer>
    </>
  );
